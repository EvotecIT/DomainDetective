...
### Domain (Default)
```powershell
Test-DDWebsite [-DomainName] <string[]> [-ExportFormat <ReportFormat[]>] [-ExportPath <string>] [-OpenInBrowser] [-ExportArtifacts] [-ArtifactsDirectory <string>] [-DisableParallel] [-ThrottleLimit <int>] [-MaxParallelism <int>] [-DnsParallelism <int>] [-DnsEndpoints <DnsEndpoint[]>] [-MultiResolverStrategy <MultiResolverStrategy>] [-MultiResolverMaxParallelism <int>] [-Port <int>] [-SkipRevocation] [<CommonParameters>]
```

## DESCRIPTION
Outputs CertificateInfo and HttpInfo views.

## EXAMPLES

### EXAMPLE 1
```powershell
Test-DDWebsite -ExportPath 'C:\Path'
```
