...
### EXAMPLE 1
```powershell
Test-DDDmarcAggregate -Path 'C:\Path'
```
...
### -Deduplicate
When set, attempts to de-duplicate reports using report-id + date range.
...
### -Json
Emit JSON for the summary to the pipeline.
...
### -Path
Path to a report file, a directory, or a wildcard pattern (supports .xml|.gz|.zip).
...
### -SummarizeBy
Summarization mode: Domain (default), Ip, or HeaderFrom.

```yaml
Type: DmarcAggregateSummarizeBy
Parameter Sets: __AllParameterSets
Aliases: None
Possible values: Domain, Ip, HeaderFrom, Reporter, Asn, Country
