...
### ByName (Default)
```powershell
Get-DDIdpInfo [-DomainName] <string[]> [-DisableParallel] [-ThrottleLimit <int>] [-MaxParallelism <int>] [-DnsParallelism <int>] [-DnsEndpoints <DnsEndpoint[]>] [-MultiResolverStrategy <MultiResolverStrategy>] [-MultiResolverMaxParallelism <int>] [<CommonParameters>]
```

## DESCRIPTION
Performs OIDC discovery and GetUserRealm calls to expose tenant hints.
...
### -DomainName
Domain(s) to probe for identity tenant information.
