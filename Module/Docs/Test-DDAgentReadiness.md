...
### __AllParameterSets
```powershell
Test-DDAgentReadiness [-Subject] <string[]> [-ScoreProfile <AgentReadinessScoreProfile>] [-TimeoutSeconds <int>] [-NoHttpFallback] [-UserAgent <string>] [-ExportFormat <ReportFormat[]>] [-ExportPath <string>] [-OpenInBrowser] [-ExportArtifacts] [-ArtifactsDirectory <string>] [-DisableParallel] [-ThrottleLimit <Int32>] [-MaxParallelism <Int32>] [-DnsParallelism <Int32>] [-DnsEndpoints <DnsEndpoint[]>] [-MultiResolverStrategy <MultiResolverStrategy>] [-MultiResolverMaxParallelism <Int32>] [<CommonParameters>]
```

## DESCRIPTION
Checks robots.txt, llms.txt, markdown alternates, Content-Signal policy, Link headers, API Catalog, Agent Skills, agents.json, OpenAPI, and trust headers.

## EXAMPLES

### EXAMPLE 1
```powershell
PS> Test-DDAgentReadiness -Subject evotec.xyz
```


### EXAMPLE 2
```powershell
PS> Test-DDAgentReadiness -Subject https://example.com/ -ScoreProfile CloudflareLike
```
...
```yaml
Type: AgentReadinessScoreProfile
Parameter Sets: __AllParameterSets
Aliases: None
Possible values: DomainDetectiveDefault, CloudflareLike, SeoAgentReadiness
...
### -Subject
Domain, host, or absolute HTTP/HTTPS URL to scan.
...
- `DomainDetective.Views.AgentReadinessInfo`
