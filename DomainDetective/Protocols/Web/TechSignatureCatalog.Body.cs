...
internal static partial class TechSignatureCatalog
{
    private static readonly (Regex regex, string tech)[] BodyRules = new[]
    {
        ...
        // gtag with property id (UA-..., G-..., GA4 variants)
        (new Regex("googletagmanager\\.com/gtag/js\\?id=([A-Z0-9-]+)", RegexOptions.IgnoreCase|RegexOptions.Compiled), "Google Analytics"),
        ...
        // GTM container id
        (new Regex("googletagmanager\\.com/gtm\\.js\\?id=(GTM-[A-Z0-9]+)", RegexOptions.IgnoreCase|RegexOptions.Compiled), "Google Tag Manager"),
        ...
        foreach (var (regex, tech) in BodyRules)
        {
            try
            {
                ...
                if (m.Success)
                {
                    outTech.Add(tech);
                    string? version = null;
                    for (int gi = 1; gi < m.Groups.Count; gi++) { var gv = m.Groups[gi].Value; if (!string.IsNullOrWhiteSpace(gv)) { version = gv; break; } }
