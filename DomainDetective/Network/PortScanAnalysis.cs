...
public class PortScanAnalysis
{
    ...
    internal static readonly Dictionary<PortScanProfile, int[]> ProfilePorts = new() {
        ...
        [PortScanProfile.DNS] = new[] { 53 },
        [PortScanProfile.LDAP] = new[] { 389, 636, 3268, 3269 },
    ...
    private static readonly Dictionary<int, Func<NetworkStream, CancellationToken, Task<string?>>[]> DetectionStrategies = new()
    {
        ...
        [53] = new[] { DetectDnsTcpAsync },
        [389] = new[] { DetectLdapAsync },
        [636] = new[] { DetectLdapsAsync },
        [3268] = new[] { DetectLdapAsync },
        [3269] = new[] { DetectLdapsAsync },
    ...
    private async Task<ScanResult> ScanPort(string host, int port, InternalLogger? logger, CancellationToken token)
    {
        ...
        else if (await DnsProbe.ProbeAsync(address.ToString(), port, Timeout, logger, token).ConfigureAwait(false))
        {
            udpOpen = true;
            banner = "DNS";
        }
        else if (await NtpProbe.ProbeAsync(address.ToString(), port, Timeout, logger, token).ConfigureAwait(false))
        {
            udpOpen = true;
            banner = "NTP";
        }
    ...
    private static async Task<string?> DetectDnsTcpAsync(NetworkStream stream, CancellationToken token)
    {
        var query = new byte[] { 0x00, 0x00, 0x12, 0x34, 0x01, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00,
            0x07, (byte)'e', (byte)'x', (byte)'a', (byte)'m', (byte)'p', (byte)'l', (byte)'e', 0x03, (byte)'c', (byte)'o', (byte)'m', 0x00, 0x00, 0x01, 0x00, 0x01 };
        query[0] = (byte)((query.Length - 2) >> 8);
        query[1] = (byte)(query.Length - 2);
        await stream.WriteAsync(query, 0, query.Length, token).ConfigureAwait(false);
        var buffer = new byte[2];
        var bytes = await stream.ReadAsync(buffer, 0, buffer.Length, token).ConfigureAwait(false);
        return bytes == 2 ? "DNS" : null;
    }

    private static async Task<string?> DetectLdapAsync(NetworkStream stream, CancellationToken token)
    {
        var ok = await LdapProbe.ProbeAsync(stream, token).ConfigureAwait(false);
        return ok ? "LDAP" : null;
    }

    private static async Task<string?> DetectLdapsAsync(NetworkStream stream, CancellationToken token)
    {
        using var ssl = new System.Net.Security.SslStream(stream, false, static (_, _, _, _) => true);
        try
        {
#if NET8_0_OR_GREATER
            await ssl.AuthenticateAsClientAsync(string.Empty, null, System.Security.Authentication.SslProtocols.Tls13 | System.Security.Authentication.SslProtocols.Tls12, false)
                .WaitWithCancellation(token);
#else
            await ssl.AuthenticateAsClientAsync(string.Empty).WaitWithCancellation(token);
#endif
            var ok = await LdapProbe.ProbeAsync(ssl, token).ConfigureAwait(false);
            return ok ? "LDAP" : null;
        }
        catch
        {
