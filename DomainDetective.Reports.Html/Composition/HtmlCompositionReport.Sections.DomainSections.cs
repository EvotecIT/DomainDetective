...
public static partial class HtmlCompositionReport {
    ...
    private static void RenderDesiredStateSection(TablerAccordion acc, DomainBucket b) {
        ...
        acc.AddItem("Desired State", item => {
            ...
            item.HeaderRight(c => {
                c.Badge(sec.DesiredErrorCount > 0 ? $"{sec.DesiredErrorCount} Error" + (sec.DesiredErrorCount > 1 ? "s" : "") : "0 Error", TablerBadgeColor.Danger, TablerBadgeVisualStyle.Light, TablerBadgeSize.Small, pill: true);
                c.Badge(sec.DesiredWarningCount > 0 ? $"{sec.DesiredWarningCount} Warning" + (sec.DesiredWarningCount > 1 ? "s" : "") : "0 Warning", TablerBadgeColor.Warning, TablerBadgeVisualStyle.Light, TablerBadgeSize.Small, pill: true);
    ...
    private static void RenderRecommendations(TablerColumn c2, IEnumerable<SectionProjectors.SimpleRecommendation>? recommendations) {
        ...
        var rows = recommendations
            .Where(r => r != null && (!string.IsNullOrWhiteSpace(r.Code) || !string.IsNullOrWhiteSpace(r.Title) || !string.IsNullOrWhiteSpace(r.How)))
        ...
        c2.Card(card => {
            card.Header(h => h.Title("Recommendations").Icon(TablerIconType.ListCheck));
            card.Body(body => {
                var t = (DataTablesTable)body.Table(rows, TableType.DataTables);
                ConfigureStandardDataTable(t, defaultMode: ToggleViewMode.ScrollX);
                t.EnablePaging(10, new[] { 10, 25, 50 })
