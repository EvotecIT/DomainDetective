...
public static partial class HtmlCompositionReport
{
        ...
        Dictionary<string, List<string>> inputSectionOrder)
    {
        page.Row(rr => rr.Column(TablerColumnNumber.Twelve, c =>
        {
            c.Card(card =>
            {
                card.Header(h => h.Title("Domains").Subtitle("Switch tabs to view each domain").Icon(TablerIconType.Stack2));
                card.Body(body =>
                {
                    body.Tabs(tabs =>
                    {
                        ...
                        foreach (var kv in ordered)
                        {
                            ...
                            var tabBadgeCount = (warnCount + errCount).ToString();
                            var tabBadgeColor = errCount > 0
                                ? TablerBadgeColor.Danger
                                : (warnCount > 0 ? TablerBadgeColor.Warning : TablerBadgeColor.Success);

                            var panel = tabs.AddTab(domain, panel =>
                            {
                                panel.Card(domainCard =>
                                {
                                    ...
                                    if (errCount > 0)
                                    {
                                        sevColor = TablerColor.Danger;
                                        statusText = $"{errCount} Error" + (errCount > 1 ? "s" : "");
                                    }
                                    else if (warnCount > 0)
                                    {
                                        sevColor = TablerColor.Warning;
                                ...
                                panel.Accordion(acc =>
                                {
                                    RenderDomainSections(acc, domain, b, sectionOrderMode, normalizedCustom, inputSectionOrder);
                                });
                            });

                            panel.WithIcon(TablerIconType.Globe)
                                 .WithBadge(tabBadgeCount, tabBadgeColor);
                        }
                    });
                });
            });
