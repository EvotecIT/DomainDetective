...
{
    private static void RenderDnsOverTlsSection(TablerAccordion acc, DomainBucket b)
    {
        var dot = b.DnsOverTls;
        if (dot == null)
        {
            return;
        }

        var sec = SectionProjectors.BuildDnsOverTls(dot);

        var warnCount = dot.WarningCount;
        var errCount = dot.ErrorCount;
        var status = dot.Status ?? "Unknown";
        ...
        acc.AddItem("DNS over TLS", item =>
        {
            ...
            item.Content(content =>
            {
                content.Row(r => r.Column(TablerColumnNumber.Twelve, c2 =>
                {
                    ...
                    }, subtitle: "Checks DNS over TLS (DoT) support on authoritative name servers.");
                    ...
                    RenderResultsTabsCard(c2, tabs =>
                    {
                        tabs.AddTab("Summary", panel =>
                        {
                            panel.Row(rr => rr.Column(TablerColumnNumber.Twelve, col =>
                            {
                                if (sec == null)
                                {
                                    col.Text("DNS over TLS section could not be projected.").Style(TablerTextStyle.Muted);
                                    return;
                                }
                        ...
                        tabs.AddTab("Endpoints", panel =>
                        {
                            panel.Row(rr => rr.Column(TablerColumnNumber.Twelve, col =>
                            {
                                if (sec == null || sec.Endpoints.Count == 0)
                                {
                                    col.Text("No endpoint probe results available.").Style(TablerTextStyle.Muted);
                                    return;
                                }

                                col.Card(card =>
                                {
                                    card.Header(h => h.Title("Name server endpoints").Icon(TablerIconType.Server2));
                                    card.Body(body =>
                                    {
                                        var rows = sec.Endpoints.Select(e => new
                                        {
                                            e.NameServerHost,
                                            e.ServerIp,
                                            e.Port,
