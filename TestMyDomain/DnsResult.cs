...
namespace TestMyDomain {
    public class DnsResult {
        ...
        public static IEnumerable<DnsResult> TranslateFromDohResponse(Response response, string type, string filter) {
            if (response.Answers == null) {
                yield break;
            }
            foreach (var answer in response.Answers) {
                // we join text to be able to filter properly on the whole string
                var data = answer.EscapedJoinedData;
                if (filter != null && !data.ToLowerInvariant().Contains(filter.ToLowerInvariant())) {
                    continue;
                }
                var dnsResult = new DnsResult {
                    Name = answer.Name,
                    Data = answer.EscapedData,
                    DataJoined = data
                };
                yield return dnsResult;
            }
