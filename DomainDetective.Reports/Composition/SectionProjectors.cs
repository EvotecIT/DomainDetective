...
public static partial class SectionProjectors
{
    ...
    public sealed class SimpleRecommendation
    {
        ...
        public SimpleRecommendation(string code, string title, string how) { Code = code; Title = title; How = how; }
    }
    ...
    // Desired State (baseline vs best-practice)
    public sealed class DesiredStateSection
    {
        ...
        public List<(string Key, string Value)> Summary { get; } = new();
        public List<SimpleFinding> DesiredFindings { get; } = new();
        public List<string> DesiredPositives { get; } = new();
        public List<SimpleRecommendation> DesiredRecommendations { get; } = new();
        public List<SimpleFinding> BestPracticeFindings { get; } = new();
        public List<string> BestPracticePositives { get; } = new();
        public List<SimpleRecommendation> BestPracticeRecommendations { get; } = new();
        public List<string> References { get; } = new();
    }
    ...
    public static DesiredStateSection? BuildDesiredState(DomainDetective.Views.DesiredStateInfo ds)
    {
        ...
        sec.Status = sec.DesiredErrorCount > 0 ? "Error"
        ...
        foreach (var p in ds.Positives ?? Array.Empty<DomainDetective.RecommendationAdvice>())
        {
            ...
            if (!string.IsNullOrWhiteSpace(t)) sec.DesiredPositives.Add(t!);
        }
        ...
        {
            var t = p?.Title ?? p?.Code;
            if (!string.IsNullOrWhiteSpace(t)) sec.BestPracticePositives.Add(t!);
        }
        ...
        foreach (var r in ds.References ?? Array.Empty<string>())
        {
            if (!string.IsNullOrWhiteSpace(r)) sec.References.Add(r);
        }
